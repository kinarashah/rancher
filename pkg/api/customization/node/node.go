...
func cordonUncordonNode(actionName string, apiContext *types.APIContext, cordon bool) error {
	...
	if cordon && unschedulable {
			...
			return httperror.NewAPIError(httperror.InvalidAction, fmt.Sprintf("Node %s already schedulable", apiContext.ID))
	}
	values.PutValue(node, !unschedulable, "unschedulable")
	_, err := schema.Store.Update(apiContext, schema, node, apiContext.ID)
