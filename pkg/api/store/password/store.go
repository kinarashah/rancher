...
func (p *PasswordStore) replacePasswords(sepData, data, existing map[string]interface{}) error {
	...
	*/
	if len(data) == 0 {
		...
		if err := p.deleteExistingSecrets(sepData, data, existing); err != nil {
			return err
		}
		return nil
	}
...
func (p *PasswordStore) deleteExistingSecrets(sepData, data, existing map[string]interface{}) error {
	/*
	...
	*/
	for sepKey, sepVal := range sepData {
		if convert.ToString(sepVal) == separator {
			...
			existArr := convert.ToMapSlice(existing[sepKey])
			if existArr == nil {
				if err := p.replacePasswords(convert.ToMapInterface(sepData[sepKey]), nil, convert.ToMapInterface(existing[sepKey])); err != nil {
					return err
				}
				continue
			}
			for _, each := range existArr {
				if err := p.replacePasswords(convert.ToMapInterface(sepData[sepKey]), nil, each); err != nil {
					return err
				}
			}
		}
