...
func (n *RKENodeConfigServer) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	...
	if isNonWorkerOnly(client.Node.Status.NodeConfig.Role) {
		...
		if client.NodeVersion != 0 {
			logrus.Infof("received nodeVersion from agent for node %s, nodeVersion %v", client.Node.Name, client.NodeVersion)

			if client.Node.Status.AppliedNodeVersion != client.NodeVersion {
				nodeCopy := client.Node.DeepCopy()
				logrus.Infof("updating node %s with nodeVersion", client.Node.Name)

				nodeCopy.Status.AppliedNodeVersion = client.NodeVersion

				_, err = n.nodes.Update(nodeCopy)
				if err != nil {
					logrus.Infof("error updating node %s %v", client.Node.Name, err)
				}
			}
		}
...
func (n *RKENodeConfigServer) nodeConfig(ctx context.Context, cluster *v3.Cluster, node *v3.Node) (*rkeworker.NodeConfig, error) {
	...
	if node.Status.AppliedNodeVersion != cluster.Status.NodeVersion {
		if nodePlan.Version == cluster.Status.NodeVersion {
			...
			logrus.Infof("sending nodeVersion for node %s version %v", node.Name, nc.NodeVersion)
			...
			if nc.AgentCheckInterval != AgentCheckIntervalDuringUpgrade {
				...
				logrus.Infof("updating %s with %v for upgrading cluster %s", node.Status.NodeName, AgentCheckIntervalDuringUpgrade, cluster.Name)
				nc.AgentCheckInterval = AgentCheckIntervalDuringUpgrade
			}
		}
	}

	if nc.AgentCheckInterval == DefaultAgentCheckInterval || nc.AgentCheckInterval == AgentCheckIntervalDuringUpgrade {
		logrus.Infof("sending %v for node %s", DefaultAgentCheckInterval, node.Status.NodeName)
	}
	return nc, nil
}
