...
func (h *handler) updateClusterStatus(_ string, cluster *v3.Cluster) (*v3.Cluster, error) {
	...
	if err != nil {
		logrus.Errorf("updateClusterStatus: error fetching provisioning cluster for %s: %v", cluster.Name, err)
...
func (h *handler) getMachineProvider(cluster *v3.Cluster, provCluster *v1.Cluster) string {
	if cluster.Status.MachineProvider != "" {
		return cluster.Status.MachineProvider
	}
	if provCluster != nil {
