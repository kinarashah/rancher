...
func (i *ingressService) generateNewService(ingress ingresswrapper.Ingress, serviceType corev1.ServiceType) (*corev1.Service, error) {
	obj, err := ingresswrapper.ToCompatIngress(ingress)
	if err != nil {
		return nil, fmt.Errorf("error generating service for ingress: %w", err)
	}
	controller := true
...
func IsServiceOwnedByIngress(ingress ingresswrapper.Ingress, service *corev1.Service) (bool, error) {
	obj, err := ingresswrapper.ToCompatIngress(ingress)
	if err != nil {
		return false, fmt.Errorf("error checking service owner: %w", err)
	}
