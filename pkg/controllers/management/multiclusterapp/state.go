...
func StartMCAppStateController(ctx context.Context, cluster *config.ManagementContext) {
	mcApps := cluster.Management.MultiClusterApps("")
...
func (m *MCAppStateController) sync(key string, mcapp *v3.MultiClusterApp) (runtime.Object, error) {
	...
	logrus.Infof("mgmt state called %s", mcapp.Name)
