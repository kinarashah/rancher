...
func (uh *upgradeHandler) workerPlan(node *v3.Node, cluster *v3.Cluster) (*v3.RKEConfigNodePlan, error) {
	...
	logrus.Infof("svcOptions %#v", svcOptions)
	plan, err := librke.New().GeneratePlan(uh.ctx, rkeConfig, infos, svcOptions)
	if err != nil {
		return nil, err
	}
...
func (uh *upgradeHandler) getServiceOptions(k8sVersion string, osType string) (map[string]interface{}, error) {
	...
	logrus.Infof("k8sVersion %s  osType %s", k8sVersion, osType)
...
func planChangedForUpgrade(newPlan *v3.RKEConfigNodePlan, oldPlan *v3.RKEConfigNodePlan) bool {
	...
	if len(newProcesses) != len(oldProcesses) {
		logrus.Infof("returning true from length comparison")
		return true
	}

	for k, newProcess := range newProcesses {
		if strings.Contains(k, services.SidekickContainerName) || strings.Contains(k, "share-mnt") {
			// don't need to upgrade if just sidekick or share-mnt changed
			continue
		}
...
func myEqual(p v3.Process, o v3.Process) bool {
	if !reflect.DeepEqual(p.Labels, o.Labels) {
		logrus.Infof("change in labels %v %v", p.Labels, o.Labels)
		return true
	}

	if !reflect.DeepEqual(p.NetworkMode, o.NetworkMode) {
		logrus.Infof("change in nmode %v %v", p.NetworkMode, o.NetworkMode)
		return true
	}

	if !reflect.DeepEqual(p.PidMode, o.PidMode) {
		logrus.Infof("change in pidmode %v %v", p.PidMode, o.PidMode)
		return true
	}

	if !reflect.DeepEqual(p.Publish, o.Publish) {
		logrus.Infof("change in publish %v %v", p.Publish, o.Publish)
		return true
	}

	if !reflect.DeepEqual(p.Privileged, o.Privileged) {
		logrus.Infof("change in privileged %v %v", p.Privileged, o.Privileged)
		return true
	}

	if !reflect.DeepEqual(p.Binds, o.Binds) {
		logrus.Infof("change in binds %#v %#v", p.Binds, o.Binds)
		for i := 0; i < len(p.Binds); i++ {
			if p.Binds[i] != o.Binds[i] {
				logrus.Infof("%v changed %v %v", i, p.Binds[i], o.Binds[i])
			}
		}
		return true
	}

	if !reflect.DeepEqual(p.VolumesFrom, o.VolumesFrom) {
		logrus.Infof("change in volumes %v %v", p.VolumesFrom, o.VolumesFrom)
		return true
	}
