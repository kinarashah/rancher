...
func (uh *upgradeHandler) upgradeCluster(cluster *v3.Cluster, nodeName string) error {
	...
	if len(notReadyMap) >= maxAllowed {
		return fmt.Errorf("cluster [%s] worker-upgrade: not enough nodes to upgrade: nodes %v notReady %v maxUnavailable %v", clusterName, filtered, notReadyMap, maxAllowed)
	}
	...
	if done == filtered {
		logrus.Debugf("cluster [%s] worker-upgrade: cluster is done upgrading, done %v len(nodes) %v", done, filtered)
...
//func getObjectQueue(l map[string]*v3.Node) chan interface{} {
//	c := make(chan interface{}, len(l))
//
//	for key := range l {
//		c <- key
//	}
...
//func (uh *upgradeHandler) assertUpdates(clusterName string, updatedForPrepare map[string]*v3.Node) error {
//	var errgrp errgroup.Group
//
//	nodeQueue := getObjectQueue(updatedForPrepare)
//
...
//				updated := false
//				for i := 0; i < 10; i++ {
//					node, err := uh.nodeLister.Get(clusterName, nodeName)
//					if err != nil {
//						return err
//					}
//					if preparingNode(node) || node.Spec.InternalNodeSpec.Unschedulable {
//						updated = true
//						break
//					}
//					//logrus.Infof("node %s is not marked for preparing, no", node.Name)
//					time.Sleep(2 * time.Second)
//				}
//				if !updated {
//					errList = append(errList, fmt.Errorf("node %s is not updated yet", nodeName))
//				}
//			}
