...
func newGlobalDNSController(ctx context.Context, mgmt *config.ManagementContext) *GDController {
	...
	serverSupportsIngressV1 := ingresswrapper.ServerSupportsIngressV1(mgmt.K8sClient)
	n := &GDController{
		ingresses: ingresswrapper.CompatClient{
			IngressClient:           mgmt.K8sClient.NetworkingV1().Ingresses(namespace.GlobalNamespace),
			IngressLegacyClient:     mgmt.K8sClient.ExtensionsV1beta1().Ingresses(namespace.GlobalNamespace),
			ServerSupportsIngressV1: serverSupportsIngressV1,
		},
