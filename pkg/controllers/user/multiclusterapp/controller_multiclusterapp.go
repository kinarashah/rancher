...
func (m *MCAppController) sync(key string, mcapp *v3.MultiClusterApp) (runtime.Object, error) {
	...
	//logrus.Infof("toUpgrade  %v", toUpgrade)
	...
	if !toUpgrade {
		//logrus.Infof("mcapp.Status revision name %s", mcapp.Status.RevisionName)
		if mcapp.Status.RevisionName == "" {
			rev, err := m.createRevision(mcapp)
			if err != nil {
				return mcapp, err
			}
			//logrus.Infof("creating revision ")
			mcapp.Status.RevisionName = rev.Name
			upd, err := m.update(mcapp)
			if err != nil {
				logrus.Infof("error update %v", err)
			}
			return upd, err
		}
		return mcapp, nil
	...
	//logrus.Infof("resp.canProceed %v", resp.canProceed)
	if !resp.canProceed {
		return mcapp, nil
	}
	updateDone := len(resp.updateApps) == 0 && v3.MultiClusterAppConditionDeployed.IsUnknown(mcapp)
	//logrus.Infof("updateDone  %v", updateDone)
	...
	//logrus.Infof("resp.remaining %v", resp.remaining)
	//logrus.Infof("len of updateApps %v", len(resp.updateApps))
...
func (m *MCAppController) toUpgrade(mcapp *v3.MultiClusterApp) (bool, error) {
	...
	if v3.MultiClusterAppConditionDeployed.IsUnknown(mcapp) {
		...
		if err != nil {
			//logrus.Infof("false couldn't get time")
			return false, err
		}
		interval := 0
