...
func (m *MCAppStateController) sync(key string, mcapp *v3.MultiClusterApp) (runtime.Object, error) {
	...
	for _, t := range mcapp.Spec.Targets {
		...
		projectNS := split[1]
		//logrus.Infof("projectNS %s", projectNS)
		if t.AppName != "" {
	...
	if checkForUpdate(installing, mcapp, v3.MultiClusterAppConditionInstalled) {
		return m.MultiClusterApps.Update(mcapp.DeepCopy())
	}

	if checkForUpdate(updating, mcapp, v3.MultiClusterAppConditionDeployed) {
		return m.MultiClusterApps.Update(mcapp.DeepCopy())
	}
...
func checkForUpdate(transitioning map[string]bool, mcapp *v3.MultiClusterApp, cond condition.Cond) bool {
	if len(transitioning) > 0 {
		...
		if !Equal(existing, transitioning) {
			cond.Unknown(mcapp)
			cond.Reason(mcapp, "updating apps")
